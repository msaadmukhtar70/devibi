...
export default function BlogClient({ post }) {
  ...
  const recent = (post.allBlogs || []).filter((b) => b.slug !== post.slug).slice(0, 3);

  // Categories with counts
  const catCounts = (post.allBlogs || []).reduce((acc, b) => {
  ...
  const idx = (post.blogs || []).findIndex((b) => b.slug === post.slug);
  const prev = (post.blogs || [])[idx - 1];
  const next = (post.blogs || [])[idx + 1];
